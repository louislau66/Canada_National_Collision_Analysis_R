...
library(plotrix)
#Read dataset
ncdb <- read.csv('//Users//yiliu//Documents//Data science//R//R Project//NCDB_1999_to_2017.csv',stringsAsFactors = FALSE)
#Exploring dataset
head(ncdb)
tail(ncdb)
names(ncdb)
...
#library(psych)
#describe(ncdb) //more statistical data of numeric columns 

#Year trend
count_by_year=c()
index=1
for(i in seq(1999,2017)){
  count_by_year[index] <- length((unique(ncdb$C_CASE[ncdb$C_YEAR==i ])))
  index=index+1
}
plot(seq(1999,2017),count_by_year,type="b",pch=21,col="blue",xlab = "Year",ylab = "Number of records")
(max(count_by_year)-min(count_by_year))/max(count_by_year)
